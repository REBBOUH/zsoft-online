...
import { Appointment, AppointmentService, DoctorService, ViewAppointmentComponent } from 'app/components';
...
export class AppointmentComponent implements OnInit, OnDestroy {
    ...
    private onSuccess(data, headers) {
        ...
        this.appointments = this.appointments.map(appointment => {
            this.userMinService.find(appointment.patientId).subscribe(response => {
                if (response.status === 200) {
                    appointment.patientName = response.body.lastName.toUpperCase() + ' ' + response.body.firstName;
                }
            });
            this.doctorService.find(appointment.doctorId).subscribe(response => {
                if (response.status === 200) {
                    const doctor = response.body;
                    this.userMinService.find(doctor.userId).subscribe(response2 => {
                        if (response2.status === 200) {
                            appointment.doctorName = response2.body.lastName.toUpperCase() + ' ' + response2.body.firstName;
                        }
                    });
                }
            });
